...
from torch.utils.data import DataLoader, Sampler, RandomSampler, SequentialSampler, sampler
...
def collate_MIL(batch):
	...
	# BATCH: idx0: Tensor of size (K, features_shape) [features]
	# idx1: tensor of size (1, num_genes)
	# idx2: array of size (K, 2) [coords]
...
def get_simple_loader(dataset, batch_size=1, num_workers=1):
	kwargs = {'num_workers': 5, 'pin_memory': False, 'num_workers': num_workers} if device.type == "cuda" else {}
	loader = DataLoader(dataset, batch_size=batch_size, sampler = sampler.SequentialSampler(dataset), collate_fn = collate_MIL, **kwargs)
	return loader 

# def return_weights():
# 	import pandas as pd
# 	df = pd.read_csv('./RNA-SEQ-Analysis/rnaseq.csv')

def get_split_loader(split_dataset, training = False):
	"""
		return either the validation loader or training loader 
	"""
	kwargs = {'num_workers': 5} if device.type == "cuda" else {}
...
def get_optim(model, args):
	if args.opt == "adam":
		...
		optimizer = optim.LBFGS(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr)
	else:
