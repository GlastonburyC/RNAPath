...
def extract_features(args, chunk, doc):
    ...
    for slidename in chunk:
        ...
        for tile, coords in loader:
            ...
            ok_patches = torch.where(cosine_similarity(WHITE_FEATURES, torch.from_numpy(features)) <= 0.70)[0].tolist()
            
            features = features[ok_patches]
            col = col[ok_patches]
            row = row[ok_patches]
